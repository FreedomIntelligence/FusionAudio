...
class GenerationMixin:
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        if audio_input != None:
            ...
            audio_att_mask = torch.ones((ori_att_mask.shape[0], 96), dtype=torch.bool, device=ori_att_mask.device)
            # audio_att_mask = torch.ones((ori_att_mask.shape[0], 64), dtype=torch.bool, device=ori_att_mask.device)
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        ```"""
        # === 处理音频输入的注意力掩码 ===
        # 处理音频输入的扩展和 batch_size
        if audio_input is not None:
            num_beams = beam_scorer.num_beams
            batch_size = input_ids.shape[0] // num_beams  # 原始 batch_size
            # 扩展音频输入的 batch_size 到 batch_size * num_beams
            audio_input = audio_input.repeat_interleave(num_beams, dim=0)
            
            # 处理注意力掩码（保持原有逻辑）
            ori_att_mask = model_kwargs['attention_mask']
            batch_size_expanded = input_ids.shape[0]  # 扩展后的 batch_size (batch_size * num_beams)
            audio_att_mask = torch.ones(
                (batch_size_expanded, 96), 
                dtype=torch.bool, 
                device=ori_att_mask.device
            )
            model_kwargs['attention_mask'] = torch.cat([ori_att_mask, audio_att_mask], dim=1)
        ...
        while True:
            ...
            # === 控制音频输入的注入 ===
            if first_step:
                current_audio_input = audio_input  # 第一次迭代传入音频输入
                first_step = False
            else:
                current_audio_input = None  # 后续迭代不传入音频输入

            # === 准备模型输入 ===
            model_inputs = self.prepare_inputs_for_generation(
                input_ids, 
                current_audio_input,  # 传入音频输入
                **model_kwargs
            )                
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        if audio_input is not None:
            ...
            # 复制音频输入以匹配beam扩展后的batch size
            audio_input = audio_input.repeat_interleave(num_beams, dim=0)
            # 调整注意力掩码
            ori_att_mask = model_kwargs['attention_mask']
            batch_size_expanded = input_ids.shape[0]
            audio_att_mask = torch.ones(
                (batch_size_expanded, 96),  # 假设音频部分长度固定为96
                dtype=torch.bool, 
                device=ori_att_mask.device
            )
            model_kwargs['attention_mask'] = torch.cat([ori_att_mask, audio_att_mask], dim=1)
        ...
        while True:
            ...
            # 新增：控制音频输入的传递
            if first_step:
                current_audio_input = audio_input
                first_step = False
            else:
                current_audio_input = None
            
            # 修改：将current_audio_input传入prepare_inputs_for_generation
            model_inputs = self.prepare_inputs_for_generation(
                input_ids, 
                current_audio_input,  # 新增参数
                **model_kwargs
            )
